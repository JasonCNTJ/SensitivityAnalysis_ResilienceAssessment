...
def GPRmodel(X_predict):
    ...
    # ridr
    param_pred = np.loadtxt('param_pred.txt')
    ...
    X_predict_ridr[:, 9] = np.exp(observed_pred_test_pidr1.mean.numpy())
    X_predict_ridr[:, 10] = np.exp(observed_pred_test_pidr2.mean.numpy())
    X_predict_ridr[:, 11] = np.exp(observed_pred_test_pidr3.mean.numpy())
    ...
    Y_predict = np.hstack((observed_pred_test_pidr1.mean.numpy()[:, np.newaxis],
